...
export default class GithubService {
    ...
    public async getFilesFromRepo(owner: string, repo: string, branch: string = 'master'): Promise<{ path: string, sha: string }[]> {
        const {
            statusCode,
            body
        } = await request(`https://ungh.cc/repos/${owner}/${repo}/files/${branch}`)
    ...
    public async getFileContent(owner: string, repo: string, path: string, branch: string = 'master'): Promise<string> {
        const {
            statusCode,
            body
        } = await request(`https://ungh.cc/repos/${owner}/${repo}/files/${branch}/${path}`)
