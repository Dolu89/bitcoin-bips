...
When two parties (later referred to as sender and receiver) want to
...
Another implementation proposal has been written: [BIP79
...
If for some reason the receiver is unable to create a payjoin proposal,
...
In some situation, the sender might want to pay some additional fee in
...
-   For each outputs in the proposal:
    ...
    -   If the output is the [fee output](#fee-output "wikilink"):
        ...
        -   Make sure the actual contribution is only paying for fee
            ...
            than or equals to
            `originalPSBTFeeRate * vsize(sender_input_type) * (count(original_psbt_inputs) - count(payjoin_proposal_inputs))`.
...
Most wallets are creating a round fee rate (like 2 sat/b). If the
...
Because the receiver needs to bump the fee to keep the same fee rate as
...
Unless disallowed by sender explicitely via
...
A receiver might run the payment server (generating the BIP21 invoice)
...
Here is pseudo code of a sender implementation. `RequestPayjoin` takes
