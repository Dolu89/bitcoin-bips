...
During the MUST\_SIGNAL and LOCKED\_IN phases, blocks that fail to
signal are invalid. For flexibility, during the LOCKED\_IN phase only,
this rule does NOT require the top 3 bits to be set any particular way.
...
Blocks received while in the MUST\_SIGNAL and LOCKED\_IN phases must be
...
for deployments in MUST\_SIGNAL and LOCKED\_IN states, to ensure blocks
produced are valid.
